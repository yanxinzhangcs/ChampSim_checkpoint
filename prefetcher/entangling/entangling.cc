...
void entangling::prefetcher_initialize() 
{
  ...
  uint32_t sets = parent_cache->NUM_SET;
  uint32_t ways = parent_cache->NUM_WAY;
...
void entangling::prefetcher_branch_operate(uint64_t ip, uint8_t branch_type, uint64_t branch_target)
{
}
...
uint32_t entangling::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint64_t instr_id, bool cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  if (!cache_hit) {
    l1i_stats_table[my_cpu][(line_addr & L1I_STATS_TABLE_MASK)].misses++;
    if (l1i_ongoing_request(line_addr)
	&& !l1i_is_accessed_timing_entry(line_addr)) {
      l1i_stats_table[my_cpu][(line_addr & L1I_STATS_TABLE_MASK)].late++;
    }
  }
  if (useful_prefetch) {
    l1i_stats_table[my_cpu][(line_addr & L1I_STATS_TABLE_MASK)].hits++;
  }
...
void entangling::prefetcher_cycle_operate()
{  
  uint32_t my_cpu = parent_cache->cpu;
  uint64_t cur_cycle = parent_cache->current_cycle();
  l1i_stats_cycle_operate++;
  l1i_stats_cycle_no_operate += (cur_cycle - (l1i_stats_last_cycle_operate[my_cpu] + 1));
  l1i_stats_last_cycle_operate[my_cpu] = cur_cycle;
}

uint32_t entangling::prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  uint32_t my_cpu = parent_cache->cpu;
  uint64_t cur_cycle = parent_cache->current_cycle();
  l1i_cpu_id = parent_cache->cpu;
  ...
  if (evicted_addr) {
    ...
    if (!accessed) {
      l1i_stats_table[my_cpu][(evicted_line_addr & L1I_STATS_TABLE_MASK)].wrong++;
    }
...
void entangling::prefetcher_final_stats()
{
  uint32_t my_cpu = parent_cache->cpu;
  std::cout << "CPU " << my_cpu << " L1I EPI prefetcher final stats" << std::endl;
