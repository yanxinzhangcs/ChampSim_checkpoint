...
SCENARIO("The ip_stride prefetcher uses branch context for instruction-style accesses")
{
  auto stride = GENERATE(as<int64_t>{}, -4, -3, -2, -1, 1, 2, 3, 4);
  GIVEN("A cache receiving requests where addr and ip are the same line")
  {
    do_nothing_MRC mock_ll;
    to_rq_MRP mock_ul;
    CACHE uut{champsim::cache_builder{champsim::defaults::default_l1d}
                  .name("452-branch-context-uut-[" + std::to_string(stride) + "]")
                  .upper_levels({&mock_ul.queues})
                  .lower_level(&mock_ll.queues)
                  .prefetcher<ip_stride>()};
    ...
    uut.impl_prefetcher_branch_operate(champsim::address{0xcafecafe}, uint8_t{}, champsim::address{});

    static uint64_t id = 1000;
    decltype(mock_ul)::request_type seed;
    seed.address = champsim::address{0xffff'003f};
    seed.v_address = seed.address;
    seed.ip = seed.address;
    seed.instr_id = id++;
    seed.cpu = 0;

    auto seed_result = mock_ul.issue(seed);
    THEN("The seed issue is accepted") { REQUIRE(seed_result); }

    for (auto i = 0; i < 100; ++i)
      for (auto elem : elements)
        elem->_operate();

    WHEN("Two more instruction-style requests stride away from the seed")
    {
      auto test_a = seed;
      test_a.address = champsim::address{champsim::block_number{seed.address} + stride};
      test_a.v_address = test_a.address;
      test_a.ip = test_a.address;
      test_a.instr_id = id++;
