...
*/
void spec_tagescl::initialize_branch_predictor() {
  //cpus.push_back(this);
  predictors.emplace_back(1);
}

bool spec_tagescl::predict_branch(std::uint64_t ip) {
  ChampsimTageScl& predictor = predictors[0]; //get_predictor(this);
...
void spec_tagescl::last_branch_result(std::uint64_t ip, std::uint64_t target,
                                bool taken, std::uint8_t branch_type) {
  ChampsimTageScl& predictor = predictors[0]; //get_predictor(this);
